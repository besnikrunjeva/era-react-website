...
function Breadcrumb({ label, lang = 'al' }) {
  return (
    <nav className="border-b border-gray-100 bg-white px-5 py-3 md:px-8">
      <div className="mx-auto flex max-w-5xl items-center gap-1.5 text-[10px] font-semibold text-gray-400">
        <Link to="/" className="transition-colors hover:text-gray-600">{lang === 'al' ? 'Ballina' : 'Home'}</Link>
        <ChevronRight className="size-3 text-gray-300" />
        <Link to="/products" className="transition-colors hover:text-gray-600">{lang === 'al' ? 'Produktet' : 'Products'}</Link>
        <ChevronRight className="size-3 text-gray-300" />
...
function SpecsSection({ specs, steps, lang = 'al' }) {
  return (
    <section className="border-b border-gray-100 bg-white px-5 py-10 md:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        >
          <div>
            <h2 className="mb-5 text-base font-black text-gray-900">{lang === 'al' ? 'Specifikimet' : 'Specifications'}</h2>
          ...
          <div>
            <h2 className="mb-5 text-base font-black text-gray-900">{lang === 'al' ? 'Si të porosisësh' : 'How to order'}</h2>
...
function RelatedSection({ related, lang = 'al' }) {
  return (
    <section className="border-b border-gray-100 bg-white px-5 py-10 md:px-8">
      <div className="mx-auto max-w-5xl">
        <h2 className="mb-5 text-base font-black text-gray-900">{lang === 'al' ? 'Produkte të ngjashme' : 'Related products'}</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {related.map(({ slug, img, al, en, sub, available }, i) => {
            const name = lang === 'al' ? al : (en || al)
            return (
              <motion.div
                key={slug}
              ...
              >
                {available ? (
                  ...
                  >
                    <div className="size-12 shrink-0 overflow-hidden rounded-lg bg-white">
                      <img src={img} alt={name} className="h-full w-full object-cover opacity-70" />
                    </div>
                    <div className="flex-1">
                      <p className="text-[12px] font-bold text-gray-700">{name}</p>
                      <p className="text-[10px] text-gray-400">{sub}</p>
...
export function Product3DPage({ lang = 'al', crumbLabel, specs, steps, related, children }) {
  return (
    <>
      <Breadcrumb label={crumbLabel} lang={lang} />
      {children}
      <SpecsSection specs={specs} steps={steps} lang={lang} />
      <RelatedSection related={related} lang={lang} />
      <BottomCTA lang={lang} />
    </>
  )
}
