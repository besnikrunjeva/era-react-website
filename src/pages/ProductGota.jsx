...
const RELATED = [
  { slug: '/products/kapak-gota', img: imgKapak,    al: 'Kapak Gote',    sub: 'PET · 3 madhësi'  },
  { slug: '/products/kupa-pasta', img: imgKupa,     al: 'Kupa Paste',    sub: 'S · M · L'         },
  { slug: '/products/akullore',   img: imgAkullore, al: 'Kupa Akullore', sub: 'H53 · H63'          },
]
  ...
  return (
    <nav className="border-b border-gray-100 bg-white px-5 py-3 md:px-8">
      <div className="mx-auto flex max-w-5xl items-center gap-1.5 text-[10px] font-semibold text-gray-400">
        ...
        <ChevronRight className="size-3 text-gray-300" />
        <span className="font-bold text-[#4ca706]">Gota Letre</span>
      </div>
    </nav>
  )
}
          ...
          {RELATED.map(({ slug, img, al, sub }, i) => (
            ...
            >
              <span
                aria-disabled
                title="Së shpejti"
                className="group flex cursor-not-allowed items-center gap-3 overflow-hidden rounded-xl border border-gray-100 bg-gray-50 p-3 transition-colors"
              >
